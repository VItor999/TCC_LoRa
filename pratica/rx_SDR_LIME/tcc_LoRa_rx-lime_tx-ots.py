...
def socket_thread_loop():
    global socket,recv_path_sim,mutex_recv_path,mutex_stop_thread,run_threads
    run = False
    while not run:
    ...
    while run:
        if socket.poll(5) != 0: # check if there is a message on the socket
            while(not mutex_recv_path.acquire(blocking=False)):
                time.sleep(MUTEX_TIME_RETRY_LOCK)
            data = socket.recv() # grab the message
            f.write_bin_line(recv_path_sim,data) #recv_path_sim
            mutex_recv_path.release()
        else:
            time.sleep(0.020) # wait 20ms and try again
        while(not mutex_stop_thread.acquire(blocking=False)):
                time.sleep(MUTEX_TIME_RETRY_LOCK)
        run = run_threads
        mutex_stop_thread.release()
...
#region ZMQSocket 
        ...
        socket.connect(_PULL_ADDR) # connect, not bind, the PUB will bind, only 1 can bind
        socket.setsockopt(zmq.SUBSCRIBE, b'') # subscribe to topic of all (needed or else it won't work)
        socket.setsockopt(zmq.RCVBUF, 16384)
#endregion
...
#region THREADS
        ...
        socket_thread = threading.Thread(target=socket_thread_loop)
        socket_thread.start()
#endregion

### SIMULATION 
        flow_graph = rx_sdr.RX_LIME()
        for cr in crs:
            for sf in s_fact:
                i=0
                for bw in band_width:
                    ...
                    while (option.lower() != 'exit' and option.lower() != 'q'):
                        ...
                        flow_graph.set_bw(bw)
                        flow_graph.set_sf(10)
                        print(flow_graph.get_sf())
                        flow_graph.set_cr(cr)
                        flow_graph.start()
