...
#region THREADS
        ...
        noise_flow = noise.noise_TX(noise=tx_power,center_freq=center_freq,noise_seed=noise_seed)
        noise_flow.start()
            ...
            for sf in s_fact:
                for bw in band_width:
                    ...
                    while (option.lower() != 'exit' and option.lower() != 'q'):
                        ...
                        command_tag = 'Setup' 
                        command_str = f"{command_tag}#{cr}#{sf}#{bw}#{send_number}#{int(tx_power)}#{message}!"
                        print(command_str)
